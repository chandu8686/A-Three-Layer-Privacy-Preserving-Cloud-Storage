...
<%
    ...
    try {
        ...
        switch (status) {
            case 1:
                try {
                    rs = st.executeQuery("select * from oregister where name='" + name + "' AND pass='" + pass + "' AND status='Yes' AND skey='" + skey + "'");
                    if (rs.next()) {
                        ...
                        response.sendRedirect("uhome1.jsp?msg=success");
                    } else {
            ...
            case 2:
                ...
                try {
                    con = DbConnection.getConnection();
                    st = con.createStatement();
                    int i = st.executeUpdate("insert into oregister(name, pass, email, dob, gen, phone, state, country, status) values ('" + name + "','" + pass + "','" + Eamil + "','" + dob + "','" + Gender + "','" + phone + "','" + State + "','" + Country + "','No')");
                    if (i != 0) {
            ...
            case 3:
                try {
                    if (name.equalsIgnoreCase("fog") && pass.equalsIgnoreCase("fog")) {
                        response.sendRedirect("chome1.jsp?msg=success");
                    } else {
